...
void FujiHeatPump::processCorrectedFrame(const uint8_t *frame) {
  ...
  uint8_t corr_room_temp = frame[6] >> 1;
    ...
    // Mirror mode/fan into member state for the new diagnostic HA entities (added 10
    // Aug 2026, Session B kickoff) -- see the getters in FujiHeatPump.h for why this
